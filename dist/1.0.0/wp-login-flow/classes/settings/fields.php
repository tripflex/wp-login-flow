		...
		<ul id="<?php echo $o[ 'name' ]; ?>-ul" class="attachments" style="<?php if ( empty( $args[ 'value' ] ) ) echo 'display:none;'; ?>">
			<li class="attachment selected" style="position: relative; left: 0px; top: 0px;">
				<div class="attachment-preview">
					...
					<a data-name="<?php echo $o[ 'name' ]; ?>" title="<?php _e('Remove', 'wp-login-flow'); ?>" href="#" class="wpjm-upload-remove check">
...
<?php
		...
		echo "<a href=\"#\" data-name=\"{$o['name']}\" data-title=\"{$o['modal_title']}\" data-button=\"{$o['modal_btn']}\" class=\"wplf-upload-show button button-secondary\">" . __( 'Select Logo', 'wp-login-flow' ) . "</a>";
		$this->description( $o );
