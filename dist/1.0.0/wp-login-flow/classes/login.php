...
//		add_filter( 'validate_password_reset', array( $this, 'activation_password_set' ), 9999, 1 );
	...
	function activation_pending_page(){
		...
		$thankyou_notice = sprintf( __( 'Thank you for registering.  Please check your email for your activation link.<br><br>If you do not receive the email please request a <a href="%s">password reset</a> to have the email sent again.', 'wp-login-flow' ), wp_lostpassword_url() );
		$template        = new WP_Login_Flow_Template();
		$notice          = $template->generate( 'wplf_notice_activation_required', $thankyou_notice );
		login_header( __( 'Pending Activation', 'wp-login-flow' ), '<p class="message reset-pass">' . $thankyou_notice . '</p>' );
		login_footer();
		exit;
		...
		<script type="text/javascript">
			document.addEventListener(
				"DOMContentLoaded", function ( event ) {
					document.getElementById( "reg_passmail" ).innerHTML = '<?php _e( 'You will be emailed a link to set your password.', 'wp-login-flow' ) ?>';
				}
			);
		</script>
