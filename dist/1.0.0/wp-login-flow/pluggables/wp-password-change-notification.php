...
if ( ! function_exists( 'wp_password_change_notification' ) ) :
	...
	function wp_password_change_notification( &$user ) {
		...
		if ( ! $activation->check( $user->ID ) ) {
			...
			login_header( __( 'Password Saved', 'wp-login-flow' ), '<p class="message reset-pass">' . get_option( 'wplf_notice_activation_thankyou' ) . '<br>You can now <a href="' . esc_url( wp_login_url() ) . '">' . __( 'Log in', 'wp-login-flow' ) . '</a></p>' );
			login_footer();
			exit;
		}
