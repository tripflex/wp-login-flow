 ...
 * Last Updated: Wed Dec 03 2014 20:05:28
 */
...
Class WP_Login_Flow {
	...
	public static function admin_notices() {
		...
		if ( WP_LOGIN_FLOW_VERSION != get_option( 'WP_LOGIN_FLOW_VERSION' ) ) {
			...
			$html .= __( 'WP Login Flow is now activated, you can find <strong>Login Flow</strong> under the <strong>Users</strong> menu.<br />Dont forget to check/enable <strong>Anyone can register</strong> under <strong>Settings > General</strong>', 'wp-login-flow' );
			$html .= '</p></div>';
		...
		if( is_multisite() && ! get_option( 'WP_LOGIN_MS_NOTICE' ) ){
			...
			$html .= '<p style="float:right;"><a href="'. esc_url( add_query_arg( "dismiss-wplf-ms-notice", "1" ) ) . '">' . __( 'Hide notice', 'wp-login-flow' ) . '</a></p>';
			$html .= '<p>' . __( 'WP Login Flow is not recommended for multisite installations.  Some features may work but other may have issues.  <strong>You have been warned.</strong>', 'wp-login-flow' );
			$html .= '</p></div>';
	...
	public function add_plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
		...
		if ( self::PLUGIN_SLUG . '/' . self::PLUGIN_SLUG . '.php' == $plugin_file ) {
			$plugin_meta[ ] = '<a target="_blank" href="http://github.com/tripflex/' . self::PLUGIN_SLUG . '">' . __( 'GitHub', 'wp-login-flow' ) . '</a>';
			$plugin_meta[ ] = '<a target="_blank" href="http://wordpress.org/plugins/' . self::PLUGIN_SLUG . '">' . __( 'Wordpress', 'wp-login-flow' ) . '</a>';
			$plugin_meta[ ] = '<a target="_blank" href="https://www.transifex.com/projects/p/' . self::PLUGIN_SLUG . '">' . __( 'Translate', 'wp-login-flow' ) . '</a>';
		}
